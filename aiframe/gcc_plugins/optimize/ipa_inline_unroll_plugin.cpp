...
void get_basic_block_info(struct cgraph_node *node, int &bb_count, int &cond_bb_count);
int get_function_parameters_info(struct cgraph_node *node);
void get_caller_callee_info(struct cgraph_node *node, int &caller_count, int &callee_count);
void get_edge_info(struct cgraph_edge *edge, int &edge_count, double &edge_freq);
...
void extract_node_features(struct cgraph_node *node, std::vector<double> &features);
void get_features(struct cgraph_node *node, struct cgraph_edge *edge, std::vector<double> &features);
...
static bool caller_growth_limits(struct cgraph_edge *e)
{
    ...
    if (newsize >= ipa_size_summaries->get(what)->size && newsize > opt_for_fn(to->decl, param_large_function_insns) &&
        (newsize > limit &&
            (edge_ignore_limits_map.find(e) == edge_ignore_limits_map.end() || !edge_ignore_limits_map[e]))) {
    ...
    stack_size_limit += ((gcov_type)stack_size_limit * opt_for_fn(to->decl, param_stack_frame_growth) / 100);

    inlined_stack =
        (ipa_get_stack_frame_offset(to) + outer_info->estimated_self_stack_size + what_info->estimated_stack_size);
...
static void report_inline_failed_reason(struct cgraph_edge *e)
{
    if (dump_enabled_p()) {
        ...
        if ((e->inline_failed == CIF_TARGET_OPTION_MISMATCH || e->inline_failed == CIF_OPTIMIZATION_MISMATCH) &&
            e->caller->lto_file_data && e->callee->ultimate_alias_target()->lto_file_data) {
            dump_printf_loc(MSG_MISSED_OPTIMIZATION,
                e->call_stmt,
                "  LTO objects: %s, %s\n",
        ...
        if (e->inline_failed == CIF_TARGET_OPTION_MISMATCH)
            if (dump_file)
                cl_target_option_print_diff(dump_file,
                    2,
        ...
        if (e->inline_failed == CIF_OPTIMIZATION_MISMATCH)
            if (dump_file)
                cl_optimization_print_diff(
                    dump_file, 2, opts_for_fn(e->caller->decl), opts_for_fn(e->callee->ultimate_alias_target()->decl));
    }
}
...
static bool sanitize_attrs_match_for_inline_p(const_tree caller, const_tree callee)
{
    ...
    const sanitize_code codes[] = {SANITIZE_ADDRESS,
        SANITIZE_THREAD,
        SANITIZE_UNDEFINED,
    ...
    for (unsigned i = 0; i < sizeof(codes) / sizeof(codes[0]); i++)
        if (sanitize_flags_p(codes[i], caller) != sanitize_flags_p(codes[i], callee))
            return false;
...
#define check_maybe_up(flag)                                                       \
    (opts_for_fn(caller->decl)->x_##flag != opts_for_fn(callee->decl)->x_##flag && \
        (!always_inline || opts_for_fn(caller->decl)->x_##flag < opts_for_fn(callee->decl)->x_##flag))
/* Used for flags where it is safe to inline when caller's value is
...
#define check_match(flag) (opts_for_fn(caller->decl)->x_##flag != opts_for_fn(callee->decl)->x_##flag)
...
static bool can_inline_edge_p(struct cgraph_edge *e, bool report, bool early = false)
{
    ...
    if (!early && (!opt_for_fn(callee->decl, optimize) || !opt_for_fn(caller->decl, optimize))) {
    ...
    else if (DECL_FUNCTION_PERSONALITY(caller->decl) && DECL_FUNCTION_PERSONALITY(callee->decl) &&
             (DECL_FUNCTION_PERSONALITY(caller->decl) != DECL_FUNCTION_PERSONALITY(callee->decl))) {
    ...
    /* Check compatibility of target optimization options.  */
        ...
        inlinable = false;
    } else if (ipa_fn_summaries->get(callee) == NULL || !ipa_fn_summaries->get(callee)->inlinable) {
...
static int inline_insns_auto(cgraph_node *n, bool hint, bool hint2)
{
    ...
    if (hint || hint2)
        return max_inline_insns_auto * opt_for_fn(n->decl, param_inline_heuristics_hint_percent) / 100;
...
static bool can_inline_edge_by_limits_p(
    struct cgraph_edge *e, bool report, bool disregard_limits = false, bool early = false)
{
    gcc_checking_assert(e->inline_failed);
    ...
    if (!DECL_DISREGARD_INLINE_LIMITS(callee->decl) && !disregard_limits &&
        !lookup_attribute("flatten", DECL_ATTRIBUTES(caller->decl)) && !caller_growth_limits(e))
        inlinable = false;
    else if (callee->externally_visible && !DECL_DISREGARD_INLINE_LIMITS(callee->decl) &&
    ...
    /* Don't inline a function with a higher optimization level than the
       ...
       optimization attribute.  */
        ...
        bool always_inline = (DECL_DISREGARD_INLINE_LIMITS(callee->decl) &&
       ...
       Not even for always_inline declared functions.  */
        else if (check_match(flag_wrapv) || check_match(flag_trapv) || check_match(flag_pcc_struct_return) ||
                 check_maybe_down(optimize_debug)
                 /* When caller or callee does FP math, be sure FP codegen flags
                compatible.  */
                 || ((caller_info->fp_expressions && callee_info->fp_expressions) &&
                        (check_maybe_up(flag_rounding_math) || check_maybe_up(flag_trapping_math) ||
                            check_maybe_down(flag_unsafe_math_optimizations) ||
                            check_maybe_down(flag_finite_math_only) || check_maybe_up(flag_signaling_nans) ||
                            check_maybe_down(flag_cx_limited_range) || check_maybe_up(flag_signed_zeros) ||
                            check_maybe_down(flag_associative_math) || check_maybe_down(flag_reciprocal_math) ||
                ...
                This is tracked by DECL_FUNCTION_PERSONALITY.  */
                 || (check_maybe_up(flag_non_call_exceptions) && DECL_FUNCTION_PERSONALITY(callee->decl)) ||
       ...
       DECL_DISREGARD_INLINE_LIMITS is set.  */
        else if ((callee->merged_comdat && !lookup_attribute("optimize", DECL_ATTRIBUTES(caller->decl))) ||
       ...
       if one of functions is not optimized at all.  */
        else if (!opt_for_fn(callee->decl, optimize) || !opt_for_fn(caller->decl, optimize)) {
            e->inline_failed = CIF_OPTIMIZATION_MISMATCH;
            inlinable = false;
        }
       ...
       This will allow caller to run faster.  */
        else if (opt_for_fn(callee->decl, optimize_size) > opt_for_fn(caller->decl, optimize_size)) {
            int growth = estimate_edge_growth(e);
            if (growth > opt_for_fn(caller->decl, param_max_inline_insns_size) &&
                (!DECL_DECLARED_INLINE_P(callee->decl) &&
                    (growth >=
                            MAX(inline_insns_single(caller, false, false), inline_insns_auto(caller, false, false)) &&
       ...
       we generally want to inline only cheap (runtime wise) functions.  */
                 ...
                 (opt_for_fn(callee->decl, optimize) > opt_for_fn(caller->decl, optimize))) {
            if (estimate_edge_time(e) >= 20 + ipa_call_summaries->get(e)->call_stmt_time) {
                e->inline_failed = CIF_OPTIMIZATION_MISMATCH;
                inlinable = false;
            }
        }
...
static bool can_early_inline_edge_p(struct cgraph_edge *e)
{
    ...
    if (!gimple_in_ssa_p(DECL_STRUCT_FUNCTION(e->caller->decl)) ||
        ...
        if (dump_enabled_p())
            dump_printf_loc(MSG_MISSED_OPTIMIZATION, e->call_stmt, "  edge not inlinable: not in SSA form\n");
        return false;
    } else if (profile_arc_flag &&
               ((lookup_attribute("no_profile_instrument_function", DECL_ATTRIBUTES(caller->decl)) == NULL_TREE) !=
                   (lookup_attribute("no_profile_instrument_function", DECL_ATTRIBUTES(callee->decl)) == NULL_TREE)))
        return false;

    if (!can_inline_edge_p(e, true, true) || !can_inline_edge_by_limits_p(e, true, false, true))
        return false;
    return true;
}
...
inline sreal compute_uninlined_call_time(struct cgraph_edge *edge, sreal uninlined_call_time, sreal freq)
{
    cgraph_node *caller = (edge->caller->inlined_to ? edge->caller->inlined_to : edge->caller);
...
inline sreal compute_inlined_call_time(struct cgraph_edge *edge, sreal time, sreal freq)
{
    cgraph_node *caller = (edge->caller->inlined_to ? edge->caller->inlined_to : edge->caller);
    sreal caller_time = ipa_fn_summaries->get(caller)->time;
...
inline sreal inlining_speedup(struct cgraph_edge *edge, sreal freq, sreal uninlined_time, sreal inlined_time)
{
...
static bool want_inline_small_function_p(struct cgraph_edge *e, bool report)
{
    ...
    if (cgraph_inline_failed_type(e->inline_failed) == CIF_FINAL_ERROR)
        ...
        ;
    else if (!DECL_DECLARED_INLINE_P(callee->decl) && !opt_for_fn(e->caller->decl, flag_inline_small_functions)) {
        e->inline_failed = CIF_FUNCTION_NOT_INLINE_CANDIDATE;
        want_inline = false;
    }
    /* Do fast and conservative check if the function can be good
       inline candidate.  */
    else if ((!DECL_DECLARED_INLINE_P(callee->decl) && (!e->count.ipa().initialized_p() || !e->maybe_hot_p())) &&
             (ipa_fn_summaries->get(callee)->min_size - ipa_call_summaries->get(e)->call_stmt_size >
                     inline_insns_auto(e->caller, true, true) &&
                 (edge_ignore_limits_map.find(e) == edge_ignore_limits_map.end() || !edge_ignore_limits_map[e]))) {
        e->inline_failed = CIF_MAX_INLINE_INSNS_AUTO_LIMIT;
        want_inline = false;
    } else if ((DECL_DECLARED_INLINE_P(callee->decl) || e->count.ipa().nonzero_p()) &&
               (ipa_fn_summaries->get(callee)->min_size - ipa_call_summaries->get(e)->call_stmt_size >
                   inline_insns_single(e->caller, true, true))) {
        e->inline_failed = (DECL_DECLARED_INLINE_P(callee->decl) ? CIF_MAX_INLINE_INSNS_SINGLE_LIMIT
       ...
       is increased even more.  */
        bool apply_hints = (hints & (INLINE_HINT_indirect_call | INLINE_HINT_known_hot | INLINE_HINT_loop_iterations |
       ...
       outcome of decision.  */
                 ...
                 (growth >= inline_insns_single(e->caller, apply_hints, apply_hints2)) &&
                 (apply_hints || apply_hints2 || growth >= inline_insns_single(e->caller, true, apply_hints2) ||
                     !big_speedup_p(e))) {
            e->inline_failed = CIF_MAX_INLINE_INSNS_SINGLE_LIMIT;
            want_inline = false;
        } else if (!DECL_DECLARED_INLINE_P(callee->decl) && !opt_for_fn(e->caller->decl, flag_inline_functions) &&
                   growth >= opt_for_fn(to->decl, param_max_inline_insns_small)) {
            /* growth_positive_p is expensive, always test it last.  */
            if ((growth >= inline_insns_single(e->caller, false, false)) || growth_positive_p(callee, e, growth)) {
       ...
       inline.  Bypass the limit when speedup seems big.  */
                 ...
                 (growth >= inline_insns_auto(e->caller, apply_hints, apply_hints2) &&
                     (edge_ignore_limits_map.find(e) == edge_ignore_limits_map.end() || !edge_ignore_limits_map[e])) &&
        ...
        /* If call is cold, do not inline when function body would grow. */
                 ...
                 ((growth >= inline_insns_single(e->caller, false, false)) || growth_positive_p(callee, e, growth)) &&
...
static bool want_inline_self_recursive_call_p(
    struct cgraph_edge *edge, struct cgraph_node *outer_node, bool peeling, int depth)
{
    char const *reason = NULL;
    bool want_inline = true;
    sreal caller_freq = 1;
    int max_depth = opt_for_fn(outer_node->decl, param_max_inline_recursive_depth_auto);
    ...
    if (!edge->maybe_hot_p()) {
        ...
        want_inline = false;
    } else if (outer_node->inlined_to && (caller_freq = outer_node->callers->sreal_frequency()) == 0) {
    ...
    /* Recursive inlining, i.e. equivalent of unrolling, is profitable if
       ...
       of libstdc++ rb tree methods.  */
        ...
        if (edge->sreal_frequency() * 100 <=
            caller_freq * opt_for_fn(outer_node->decl, param_min_inline_recursive_probability)) {
            reason = "frequency of recursive call is too small";
            want_inline = false;
        }
    }
    if (!want_inline && dump_enabled_p())
        dump_printf_loc(MSG_MISSED_OPTIMIZATION, edge->call_stmt, "   not inlining recursively: %s\n", reason);
    return want_inline;
}
...
static bool check_callers(struct cgraph_node *node, void *has_hot_call)
{
    ...
    for (e = node->callers; e; e = e->next_caller) {
        if (!opt_for_fn(e->caller->decl, flag_inline_functions_called_once) || !opt_for_fn(e->caller->decl, optimize))
            return true;
       ...
       of register pressure it implies.  */
        if ((int)ipa_call_summaries->get(e)->loop_depth > param_inline_functions_called_once_loop_depth)
            return true;
        /* Do not produce gigantic functions.  */
        if (estimate_size_after_inlining(e->caller->inlined_to ? e->caller->inlined_to : e->caller, e) >
...
static bool has_caller_p(struct cgraph_node *node, void *data ATTRIBUTE_UNUSED)
{
    if (node->callers)
        return true;
    return false;
...
{
    return size < (DECL_DECLARED_INLINE_P(where->decl) ? inline_insns_single(where, false, false)
...
static sreal edge_badness(struct cgraph_edge *edge, bool dump)
{
    ...
    gcc_checking_assert(
        (edge_time * 100 - callee_info->time * 101).to_int() <= 0 || callee->count.ipa().initialized_p());
    gcc_checking_assert(growth <= ipa_size_summaries->get(callee)->size);

    if (dump) {
        fprintf(
            dump_file, "    Badness calculation for %s -> %s\n", edge->caller->dump_name(), edge->callee->dump_name());
        fprintf(dump_file,
    ...
    /* When profile is available. Compute badness as:
       ...
       then calls without.
    */
    else if (opt_for_fn(caller->decl, flag_guess_branch_prob) || caller->count.ipa().nonzero_p()) {
      ...
      */
        if (growth > overall_growth
            ...
            && ((!DECL_DECLARED_INLINE_P(edge->callee->decl) && DECL_DECLARED_INLINE_P(caller->decl))
                   /* ... or when early optimizers decided to split and edge
                  frequency still indicates splitting is a win ... */
                   || (callee->split_part && !caller->split_part &&
                          freq * 100 < opt_for_fn(caller->decl, param_partial_inlining_entry_probability)
            ...
            if (!caller_info->single_caller && overall_growth < caller_growth && caller_info->inlinable &&
                wrapper_heuristics_may_apply(caller, ipa_size_summaries->get(caller)->size)) {
                if (dump)
                    fprintf(
                        dump_file, "     Wrapper penalty. Increasing growth %i to %i\n", overall_growth, caller_growth);
                overall_growth = caller_growth;
            }
        }
        if (overall_growth > 0) {
        ...
        if (dump) {
            fprintf(dump_file,
                "      %f: guessed profile. frequency %f, count %" PRId64 " caller count %" PRId64 " time saved %f"
    ...
    if ((hints & (INLINE_HINT_indirect_call | INLINE_HINT_loop_iterations | INLINE_HINT_loop_stride)) ||
...
static inline void update_edge_key(edge_heap_t *heap, struct cgraph_edge *edge)
{
...
static void update_caller_keys(
    edge_heap_t *heap, struct cgraph_node *node, bitmap updated_nodes, struct cgraph_edge *check_inlinablity_for)
{
    struct cgraph_edge *edge;
    struct ipa_ref *ref;

    if ((!node->alias && !ipa_fn_summaries->get(node)->inlinable) || node->inlined_to)
        return;
    ...
    for (edge = node->callers; edge; edge = edge->next_caller)
        if (edge->inline_failed) {
            if (!check_inlinablity_for || check_inlinablity_for == edge) {
                if (can_inline_edge_p(edge, false) && want_inline_small_function_p(edge, false) &&
...
static void update_callee_keys(
    edge_heap_t *heap, struct cgraph_node *node, struct cgraph_node *update_since, bitmap updated_nodes)
{
    struct cgraph_edge *e = node->callees;
    ...
    while (true)
        if (!e->inline_failed && e->callee->callees) {
            ...
            if (!check_inlinability) {
                if (e->aux &&
                    !bitmap_bit_p(updated_nodes, e->callee->ultimate_alias_target(&avail, e->caller)->get_uid()))
            ...
            /* We do not reset callee growth cache here.  Since we added a new call,
               growth should have just increased and consequently badness metric
                   don't need updating.  */
            else if (e->inline_failed && (callee = e->callee->ultimate_alias_target(&avail, e->caller)) &&
                     avail >= AVAIL_AVAILABLE && ipa_fn_summaries->get(callee) != NULL &&
                     ipa_fn_summaries->get(callee)->inlinable && !bitmap_bit_p(updated_nodes, callee->get_uid())) {
                if (can_inline_edge_p(e, false) && want_inline_small_function_p(e, false) &&
...
static void lookup_recursive_calls(struct cgraph_node *node, struct cgraph_node *where, edge_heap_t *heap)
{
    ...
    for (e = where->callees; e; e = e->next_callee)
        if (e->callee == node ||
            (e->callee->ultimate_alias_target(&avail, e->caller) == node && avail > AVAIL_INTERPOSABLE))
...
{
    cgraph_node *to = (edge->caller->inlined_to ? edge->caller->inlined_to : edge->caller);
    ...
    while (!heap.empty()) {
        ...
        if (!can_inline_edge_p(curr, true) || !can_inline_edge_by_limits_p(curr, true))
            continue;
       ...
       the already modified body.  */
        ...
        depth = 1;
        for (cnode = curr->caller; cnode->inlined_to; cnode = cnode->callers->caller)
            if (node->decl == curr->callee->ultimate_alias_target()->decl)
                depth++;
        ...
        if (!master_clone) {
            ...
            master_clone = node->create_clone(node->decl, node->count, false, vNULL, true, NULL, NULL);
...
static int64_t compute_max_insns(cgraph_node *node, int insns)
{
    ...
    return ((int64_t)max_insns * (100 + opt_for_fn(node->decl, param_inline_unit_growth)) / 100);
}
...
static void add_new_edges_to_heap(edge_heap_t *heap, vec<cgraph_edge *> &new_edges)
{
    while (new_edges.length() > 0) {
        ...
        if (edge->inline_failed && can_inline_edge_p(edge, true) && want_inline_small_function_p(edge, true) &&
...
bool speculation_useful_p(struct cgraph_edge *e, bool anticipate_inlining)
{
    ...
    enum availability avail;
    struct cgraph_node *target = e->callee->ultimate_alias_target(&avail, e->caller);
    ...
    if (avail >= AVAIL_AVAILABLE) {
        ...
        if (ecf_flags & ECF_CONST) {
            if (!(e->speculative_call_indirect_edge()->indirect_info->ecf_flags & ECF_CONST))
                return true;
        } else if (ecf_flags & ECF_PURE) {
            if (!(e->speculative_call_indirect_edge()->indirect_info->ecf_flags & ECF_PURE))
                return true;
        }
    }
    ...
    if (!can_inline_edge_p(e, false) || !can_inline_edge_by_limits_p(e, false, true))
        return false;
...
{
    return (!DECL_EXTERNAL(node->decl) && !opt_for_fn(node->decl, optimize_size) &&
...
static void inline_small_functions(void)
{
    ...
    edge_removal_hook_holder = symtab->add_edge_removal_hook(&heap_edge_removal_hook, &edge_heap);
    ...
    if (!node->inlined_to) {
        if (!node->alias && node->analyzed && (node->has_gimple_body_p() || node->thunk) &&
            ...
            if (dfs && dfs->next_cycle) {
                ...
                for (n2 = node; n2; n2 = ((struct ipa_dfs_info *)n2->aux)->next_cycle)
                    if (opt_for_fn(n2->decl, optimize)) {
                        ipa_fn_summary *info2 = ipa_fn_summaries->get(n2->inlined_to ? n2->inlined_to : n2);
                        if (info2->scc_no)
                            break;
                        info2->scc_no = id;
                    }
    ...
    FOR_EACH_DEFINED_FUNCTION(node)
    {
        ...
        for (edge = node->callees; edge; edge = edge->next_callee) {
            ...
            /* Invoke the plug-in framework to perform model inference and modify heuristic rules based on the
             * prediction result. */
            ...
            if (edge->inline_failed && !edge->aux && can_inline_edge_p(edge, true) &&
                want_inline_small_function_p(edge, true) && can_inline_edge_by_limits_p(edge, true) &&
                edge->inline_failed) {
                gcc_assert(!edge->aux);
    ...
    gcc_assert(in_lto_p || !(max_count > 0) || (profile_info && flag_branch_probabilities));
    ...
    while (!edge_heap.empty()) {
       ...
       IPA this we skip it in that case.  */
        if (flag_checking && !edge->callee->count.ipa_p() && (!max_count.initialized_p() || !max_count.nonzero_p())) {
            ...
            if (edge_growth_cache != NULL)
                edge_growth_cache->remove(edge);
            reset_node_cache(edge->caller->inlined_to ? edge->caller->inlined_to : edge->caller);
        ...
        if (!can_inline_edge_p(edge, true) || !can_inline_edge_by_limits_p(edge, true)) {
        ...
        if (dump_file) {
            ...
            fprintf(dump_file,
                ...
                edge->caller->dump_name(),
                edge->call_stmt &&
                        (LOCATION_LOCUS(gimple_location((const gimple *)edge->call_stmt)) > BUILTINS_LOCATION)
        ...
        if (overall_size + growth > compute_max_insns(where, min_size) &&
            (edge_ignore_limits_map.find(edge) == edge_ignore_limits_map.end() || !edge_ignore_limits_map[edge]) &&
            !DECL_DISREGARD_INLINE_LIMITS(callee->decl)) {
       ...
       specific inliner.  */
        if (edge->recursive_p()) {
            ...
            if (!recursive_inlining(
                    edge, opt_for_fn(edge->caller->decl, flag_indirect_inlining) ? &new_indirect_edges : NULL)) {
            ...
            if (outer_node && !want_inline_self_recursive_call_p(edge, outer_node, true, depth)) {
                edge->inline_failed =
                    (DECL_DISREGARD_INLINE_LIMITS(edge->callee->decl) ? CIF_RECURSIVE_INLINING : CIF_UNSPECIFIED);
                resolve_noninline_speculation(&edge_heap, edge);
                continue;
            ...
            if (old_size <= ipa_size_summaries->get(where)->size &&
                ...
                update_callee_keys(&edge_heap, edge->callee, edge->callee, updated_nodes);
            else
       ...
       available.  */
        struct cgraph_node *n = cgraph_node::get(edge->callee->decl)->ultimate_alias_target();
        if (n != edge->callee && n->analyzed && !(n->count == old_count) && n->count.ipa_p())
        ...
        if (dump_enabled_p()) {
            ...
            snprintf(buf_net_change, sizeof buf_net_change, "%+i", overall_size - old_size);
...
static void flatten_function(struct cgraph_node *node, bool early, bool update)
{
    ...
    for (e = node->callees; e; e = e->next_callee) {
       ...
       too.  */
        if (!early ? !can_inline_edge_p(e, true) && !can_inline_edge_by_limits_p(e, true) : !can_early_inline_edge_p(e))
            continue;

        if (e->recursive_p()) {
            if (dump_enabled_p())
                dump_printf_loc(MSG_MISSED_OPTIMIZATION, e->call_stmt, "Not inlining: recursive call.\n");
            continue;
        }

        if (gimple_in_ssa_p(DECL_STRUCT_FUNCTION(node->decl)) != gimple_in_ssa_p(DECL_STRUCT_FUNCTION(callee->decl))) {
            if (dump_enabled_p())
                dump_printf_loc(MSG_MISSED_OPTIMIZATION, e->call_stmt, "Not inlining: SSA form does not match.\n");
            continue;
        }
        ...
        if (dump_enabled_p())
            dump_printf_loc(MSG_OPTIMIZED_LOCATIONS, e->call_stmt, " Inlining %C into %C.\n", callee, e->caller);
        orig_callee = callee;
...
static bool inline_to_all_callers_1(struct cgraph_node *node, void *data, hash_set<cgraph_node *> *callers)
{
    ...
    while (node->callers && !node->inlined_to) {
        ...
        if (!can_inline_edge_p(node->callers, true) || !can_inline_edge_by_limits_p(node->callers, true) ||
        ...
        if (dump_file) {
            ...
            fprintf(
                dump_file, "\nInlining %s size %i.\n", ultimate->dump_name(), ipa_size_summaries->get(ultimate)->size);
            fprintf(dump_file,
...
static bool inline_to_all_callers(struct cgraph_node *node, void *data)
{
    ...
    for (hash_set<cgraph_node *>::iterator i = callers.begin(); i != callers.end(); ++i)
...
static void dump_inline_stats(void)
{
    ...
    for (i = 0; i < CIF_N_REASONS; i++)
        if (reason[i][1])
            fprintf(dump_file,
                "%-50s: %8i calls, %8f freq, %" PRId64 " count\n",
                cgraph_inline_failed_string((cgraph_inline_failed_t)i),
                (int)reason[i][1],
                reason_freq[i].to_double(),
                reason[i][0]);
}
...
static void flatten_remove_node_hook(struct cgraph_node *node, void *data)
{
    ...
    hash_set<struct cgraph_node *> *removed = (hash_set<struct cgraph_node *> *)data;
    removed->add(node);
}
...
/* Obtains the current node information, which is used to extract the caller and callee features. */
void extract_node_features(struct cgraph_node *node, std::vector<double> &features)
{
    int Caller_Count;
    int Callee_Count;
    ...
    const enum gimple_code gimple_codes[] = {GIMPLE_COND,
        GIMPLE_DEBUG,
        GIMPLE_LABEL,
        GIMPLE_SWITCH,
        GIMPLE_ASSIGN,
...
static unsigned int ipa_inline(void)
{
    ...
    for (i = nnodes - 1, j = i; i >= 0; i--) {
        ...
        if (node->definition
            ...
            && !node->alias && lookup_attribute("flatten", DECL_ATTRIBUTES(node->decl)) != NULL)
            order[j--] = order[i];
    }
    ...
    if (j < nnodes - 2) {
        ...
        node_removal_hook_holder = symtab->add_cgraph_removal_hook(&flatten_remove_node_hook, flatten_removed_nodes);
    }
    ...
    for (cold = 0; cold <= 1; cold++) {
        FOR_EACH_DEFINED_FUNCTION(node)
        {
            ...
            if (!opt_for_fn(node->decl, optimize) || !opt_for_fn(node->decl, flag_inline_functions_called_once))
                continue;
            ...
            if (want_inline_function_to_all_callers_p(node, cold)) {
                int num_calls = 0;
                node->call_for_symbol_and_aliases(sum_callers, &num_calls, true);
                while (node->call_for_symbol_and_aliases(inline_to_all_callers, &num_calls, true))
                    ;
                remove_functions = true;
            }
        }
...
static void report_unroll(class loop *loop, dump_location_t locus)
{
    ...
    dump_printf_loc(metadata, locus.get_user_location(), "loop unrolled %d times", loop->lpt_decision.times);
    if (profile_info && loop->header->count.initialized_p())
        dump_printf(metadata, " (header execution count %d)", (int)loop->header->count.to_gcov_type());
...
int64_t get_unroll_factor(class loop *loop_)
{
    ...
    int vec_size = features.size();

    initialize(g_unroll_model_path);

    add_float_input(features.data(), vec_size);
...
static void decide_unrolling_(int flags)
{
    ...
    for (auto loop : loops_list(cfun, LI_FROM_INNERMOST)) {
        ...
        if (dump_enabled_p())
            dump_printf_loc(
                MSG_NOTE, locus, "considering unrolling loop %d at BB %d\n", loop->num, loop->header->index);
...
static void decide_unroll_constant_iterations(class loop *loop, int flags)
{
    ...
    if (desc->niter < 2 * nunroll ||
        ((get_estimated_loop_iterations(loop, &iterations) || get_likely_max_loop_iterations(loop, &iterations)) &&
    ...
    if (!exit_at_end) {
       ...
       of exit condition have continuous body after unrolling.  */
        ...
        if (exit_mod) {
            ...
            ok = duplicate_loop_body_to_header_edge(loop,
                loop_preheader_edge(loop),
                exit_mod,
                wont_exit,
                desc->out_edge,
                &remove_edges,
                DLTHE_FLAG_UPDATE_FREQ | (opt_info && exit_mod > 1 ? DLTHE_RECORD_COPY_NUMBER : 0));
            gcc_assert(ok);
       ...
       exit_mod + 1 iterations.  */
        if (exit_mod != max_unroll || desc->noloop_assumptions) {
            ...
            ok = duplicate_loop_body_to_header_edge(loop,
                ...
                desc->out_edge,
                &remove_edges,
                DLTHE_FLAG_UPDATE_FREQ | (opt_info && exit_mod > 0 ? DLTHE_RECORD_COPY_NUMBER : 0));
            gcc_assert(ok);
...
static rtx_insn *compare_and_jump_seq(
    rtx op0, rtx op1, enum rtx_code comp, rtx_code_label *label, profile_probability prob, rtx_insn *cinsn)
{
    rtx_insn *seq;
    rtx_jump_insn *jump;
    rtx cond;
    ...
    if (GET_MODE_CLASS(mode) == MODE_CC) {
       ...
       conditional jump from a ccmode comparison.  */
        ...
        do_compare_rtx_and_jump(op0, op1, comp, 0, mode, NULL_RTX, NULL, label, profile_probability::uninitialized());
        jump = as_a<rtx_jump_insn *>(get_last_insn());
...
static void unroll_loop_runtime_iterations(class loop *loop)
{
    ...
    if (exit_at_end && !desc->noloop_assumptions) {
        niter = expand_simple_binop(desc->mode, PLUS, niter, const1_rtx, NULL_RTX, 0, OPTAB_LIB_WIDEN);
        old_niter = niter;
    }
    ...
    niter =
        expand_simple_binop(desc->mode, AND, niter, gen_int_mode(max_unroll, desc->mode), NULL_RTX, 0, OPTAB_LIB_WIDEN);
    ...
    if (extra_zero_check || desc->noloop_assumptions) {
       ...
       check.  */
        ...
        ok = duplicate_loop_body_to_header_edge(
            loop, loop_preheader_edge(loop), 1, wont_exit, desc->out_edge, &remove_edges, DLTHE_FLAG_UPDATE_FREQ);
        gcc_assert(ok);
    }
    ...
    for (i = 0; i < n_peel; i++) {
        ...
        branch_code =
            compare_and_jump_seq(copy_rtx(niter), gen_int_mode(j, desc->mode), EQ, block_label(preheader), p, NULL);
       ...
       and hence the cfg we create is correct.  */
        ...
        swtch->count = new_count;
        e = make_edge(swtch, preheader, single_succ_edge(swtch)->flags & EDGE_IRREDUCIBLE_LOOP);
        e->probability = p;
    }
    ...
    if (extra_zero_check) {
       ...
       have exited and reduced those values that were computed above.  */
        ...
        /* Add in count of edge from switch block.  */
        preheader->count += iter_count;
        branch_code = compare_and_jump_seq(copy_rtx(niter), const0_rtx, EQ, block_label(preheader), p, NULL);
        gcc_assert(branch_code != NULL_RTX);
    ...
    desc->niter_expr = simplify_gen_binary(UDIV, desc->mode, old_niter, gen_int_mode(max_unroll + 1, desc->mode));
    loop->nb_iterations_upper_bound = wi::udiv_trunc(loop->nb_iterations_upper_bound, max_unroll + 1);
    if (loop->any_estimate)
        loop->nb_iterations_estimate = wi::udiv_trunc(loop->nb_iterations_estimate, max_unroll + 1);
    if (loop->any_likely_upper_bound)
        loop->nb_iterations_likely_upper_bound = wi::udiv_trunc(loop->nb_iterations_likely_upper_bound, max_unroll + 1);
    if (exit_at_end) {
        desc->niter_expr = simplify_gen_binary(MINUS, desc->mode, desc->niter_expr, const1_rtx);
        desc->noloop_assumptions = NULL_RTX;
...
static bool referenced_in_one_insn_in_loop_p(class loop *loop, rtx reg, int *debug_uses)
{
    basic_block *body, bb;
    unsigned i;
    int count_ref = 0;
...
static void reset_debug_uses_in_loop(class loop *loop, rtx reg, int debug_uses)
{
    ...
    for (i = 0; debug_uses && i < loop->num_nodes; i++) {
        ...
        if (!DEBUG_INSN_P(insn) || !rtx_referenced_p(reg, insn))
            ...
            validate_change(insn, &INSN_VAR_LOCATION_LOC(insn), gen_rtx_UNKNOWN_VAR_LOC(), 0);
            if (!--debug_uses)
                break;
        }
    }
...
static struct var_to_expand *analyze_insn_to_expand_var(class loop *loop, rtx_insn *insn)
{
    rtx set, dest, src;
...
static void split_iv(struct iv_to_split *ivts, rtx_insn *insn, unsigned delta)
{
    ...
    if (!delta)
        ...
        incr = simplify_gen_binary(MULT, mode, copy_rtx(ivts->step), gen_int_mode(delta, mode));
...
static void expand_var_during_unrolling(struct var_to_expand *ve, rtx_insn *insn)
{
    rtx new_reg, set;
...
static void insert_var_expansion_initialization(struct var_to_expand *ve, basic_block place)
{
    rtx_insn *seq;
    rtx var, zero_init;
    unsigned i;
        ...
        case MINUS:
            FOR_EACH_VEC_ELT(ve->var_expansions, i, var)
            {
                if (honor_signed_zero_p)
                    zero_init = simplify_gen_unary(rtx_code::NEG, mode, CONST0_RTX(mode), mode);
                else
...
static void combine_var_copies_in_loop_exit(struct var_to_expand *ve, basic_block place)
{
    rtx sum = ve->reg;
    rtx expr, var;
    rtx_insn *seq, *insn;
...
static void apply_opt_in_copies(
    struct opt_info *opt_info, unsigned n_copies, bool unrolling, bool rewrite_original_loop)
{
    unsigned i, delta;
    ...
    for (i = opt_info->first_new_block; i < (unsigned)last_basic_block_for_fn(cfun); i++) {
       ...
       duplicate_loop_body_to_header_edge.  */
        ...
        {
            if (!INSN_P(insn) || (DEBUG_BIND_INSN_P(insn) && INSN_VAR_LOCATION_DECL(insn) &&
                                     TREE_CODE(INSN_VAR_LOCATION_DECL(insn)) == LABEL_DECL))
                continue;

            while (!INSN_P(orig_insn) || (DEBUG_BIND_INSN_P(orig_insn) && INSN_VAR_LOCATION_DECL(orig_insn) &&
    ...
    for (i = opt_info->first_new_block; i < (unsigned)last_basic_block_for_fn(cfun); i++) {
        ...
        for (orig_insn = BB_HEAD(orig_bb); orig_insn != NEXT_INSN(BB_END(bb)); orig_insn = next) {
            ...
            if (opt_info->insns_to_split) {
                ...
                ivts = (struct iv_to_split *)opt_info->insns_to_split->find(&ivts_templ);
                if (ivts) {
                    if (!delta)
...
/* ===------------------------- Ipa-Inline -------------------------=== */
const pass_data ipa_inline_opt_pass_data = {.type = IPA_PASS,  // GIMPLE_PASS, SIMPLE_IPA_PASS, RTL_PASS
...
public:
    ...
    virtual ipa_inline_opt_pass *clone() override
    {
        return this;
    }
};
...
struct register_pass_info ipa_inline_opt_passinfo {
    ...
    .pass = new ipa_inline_opt_pass(), .reference_pass_name = "inline", .ref_pass_instance_number = 0,
...
public:
    ...
    virtual rtl_unroll_opt_pass *clone() override
    {
        return this;
    }
};
...
struct register_pass_info rtl_unroll_opt_passinfo {
    .pass = new rtl_unroll_opt_pass(), .reference_pass_name = "loop2_unroll", .ref_pass_instance_number = 0,
...
int plugin_init(struct plugin_name_args *plugin_info, struct plugin_gcc_version *version)
{
    ...
    add_double_input = (add_double_input_t)dlsym(g_infer_handle, "add_double_input");
    ...
    get_int64_output = (get_int64_output_t)dlsym(g_infer_handle, "get_int64_output");
    ...
    if (inline_switch) {
        register_callback(plugin_name, PLUGIN_PASS_MANAGER_SETUP, NULL, &ipa_inline_opt_passinfo);
    }

    if (unroll_switch) {
        register_callback(plugin_name, PLUGIN_PASS_MANAGER_SETUP, NULL, &rtl_unroll_opt_passinfo);
    }
