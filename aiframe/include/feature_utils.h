...
void count_rtx_types(rtx x, std::unordered_map<enum rtx_code, unsigned int> &type_counts)
{
    if (!x)
        return;
...
static std::unordered_map<enum rtx_code, unsigned int> count_loop_insn_types(class loop *loop)
{
    basic_block *bbs, bb;
    unsigned i;
    rtx_insn *insn;
...
public:
    ...
    void analyze_insns(AutoTuning &auto_tuning) {
      ...
      for (unsigned i = 0; i < loop_->num_nodes; i++) {
        ...
        FOR_BB_INSNS (bb, insn) {
          ...
          if (!is_valid_insn_attr_type (insn))
              continue;
          ...
          if (is_store_op (ia_type))
              st_insn_num++;

          if (is_fp_compute_op (ia_type))
              fp_compute_num++;
          
          if (is_fadd_op(insn, ia_type))
              fadd_num++;
          
          if (is_fcmp_op(ia_type))
              fcmp_num++;
          
          if (is_fdiv_op(ia_type))
              fdiv_num++;
          
          if (is_fmul_op(ia_type))
              fmul_num++;

          if (is_fp2si_op(ia_type))
              fp2si_num++;
          ...
          if (is_unreachable_op(ia_type))
              unreachable_num++;

          if (is_branch_op(ia_type)) 
              br_num++;
          
          if (is_memory_op(ia_type))
              mem_num++;
          }
      }
      free (bbs);
      ...
      auto_tuning.extra_features.fs.push_back(
          ExtraFeature("fcmp_num", fcmp_num));
      auto_tuning.extra_features.fs.push_back(
          ExtraFeature("fdiv_num", fdiv_num));
      auto_tuning.extra_features.fs.push_back(
          ExtraFeature("fmul_num", fmul_num));
      auto_tuning.extra_features.fs.push_back(
          ExtraFeature("fp2si_num", fp2si_num));
      auto_tuning.extra_features.fs.push_back(
          ExtraFeature("fsub_insn", fsub_num));
      auto_tuning.extra_features.fs.push_back(
          ExtraFeature("ret_insn", ret_num));
      auto_tuning.extra_features.fs.push_back(
          ExtraFeature("unreachable_insn", unreachable_num));
      auto_tuning.extra_features.fs.push_back(
          ExtraFeature("br_insn", br_num));
      auto_tuning.extra_features.fs.push_back(
          ExtraFeature("mem_insn", mem_num));
    }
    ...
    std::vector<float> analyze_insns() {
        ...
        vec.push_back(niter);
        vec.push_back(depth);
        std::unordered_set<std::string> parm_set = {"vec_select", "mult", "neg","minus", "plus", "mem", "subreg", "reg", "const_int", "unspec", "clobber", "sign_extend"};
...
private:
    ...
    float niter{0};
    float depth{1};
    std::unordered_map<enum rtx_code, unsigned int> res = count_loop_insn_types(loop_);
};
